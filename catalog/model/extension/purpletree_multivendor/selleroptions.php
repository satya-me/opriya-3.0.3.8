<?php
class ModelExtensionPurpletreeMultivendorSelleroptions extends Model {
		public function addSellerOption($seller_id,$data) {
			$this->db->query("INSERT INTO `" . DB_PREFIX . "option` SET type = '" . $this->db->escape($data['type']) . "', sort_order = '" . (int)$data['sort_order'] . "'");
			
			$option_id = $this->db->getLastId();
			
			$this->db->query("INSERT INTO `" . DB_PREFIX . "purpletree_vendor_option` SET seller_id = '" . (int)$seller_id . "', option_id = '" . (int)$option_id . "'");
		...
		public function editSellerOption($option_id, $data) {
		...
		public function deleteSellerAttributeGroup($option_id) {
			$this->db->query("DELETE FROM `" . DB_PREFIX . "option` WHERE option_id = '" . (int)$option_id . "'");
			$this->db->query("DELETE FROM " . DB_PREFIX . "option_description WHERE option_id = '" . (int)$option_id . "'");
			$this->db->query("DELETE FROM " . DB_PREFIX . "option_value WHERE option_id = '" . (int)$option_id . "'");
			$this->db->query("DELETE FROM " . DB_PREFIX . "option_value_description WHERE option_id = '" . (int)$option_id . "'");
			$this->db->query("DELETE FROM " . DB_PREFIX . "purpletree_vendor_option WHERE option_id = '" . (int)$option_id . "'");
		}
		
		public function getSellerOption($option_id) {
			$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "option` o JOIN " . DB_PREFIX . "option_description od ON (o.option_id = od.option_id) JOIN " . DB_PREFIX . "purpletree_vendor_option pvo ON (o.option_id = pvo.option_id) WHERE o.option_id = '" . (int)$option_id . "' AND od.language_id = '" . (int)$this->config->get('config_language_id') . "'");
			
			return $query->row;
		}
		
		public function getSellerOptions($seller_id,$data = array()) {
			$sql = "SELECT * FROM `" . DB_PREFIX . "option` o LEFT JOIN " . DB_PREFIX . "option_description od ON (o.option_id = od.option_id ) LEFT JOIN " . DB_PREFIX . "purpletree_vendor_option pvo ON (o.option_id = pvo.option_id) WHERE od.language_id = '" . (int)$this->config->get('config_language_id') . "' AND pvo.seller_id = '" . (int)$seller_id . "'";
		...
		public function getSellerOptionValueDescriptions($option_id) {
			$option_value_data = array();
		...
		public function getSellerOptionDescriptions($option_id) {
			$option_data = array();
		...
		public function getTotalselleroptions($seller_id) {
			$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "purpletree_vendor_option WHERE seller_id = '" . (int)$seller_id . "'");
