<?php
class ModelExtensionPurpletreeMultivendorSellercoupons extends Model {
		public function addCoupon($seller_id,$data) {
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "purpletree_vendor_coupons SET coupon_id = '" . $coupon_id . "', seller_id = '".$seller_id."'");
		...
		public function getCoupons($seller_id,$data = array()) {
			$sql = "SELECT pvc.coupon_id, c.name, c.code, c.discount, c.date_start, c.date_end, c.status FROM " . DB_PREFIX . "purpletree_vendor_coupons pvc JOIN " . DB_PREFIX . "coupon c ON(c.coupon_id=pvc.coupon_id) WHERE pvc.seller_id ='".(int)$seller_id."'";
		...
		public function getTotalCoupons($seller_id) {
			$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "purpletree_vendor_coupons WHERE seller_id ='".$seller_id."'");
