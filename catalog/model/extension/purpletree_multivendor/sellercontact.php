...
class ModelExtensionPurpletreeMultivendorSellercontact extends Model{
		...
		public function getSellerContactCustomerschat122($seller_id,$custid){
			...
			$sql = "SELECT * FROM " . DB_PREFIX . "purpletree_vendor_contact WHERE (seller_id = '".(int)$seller_id."' and customer_id='". (int)$custid ."') OR (seller_id = '".(int)$custid."' and customer_id='". (int)$seller_id ."')";
		...
		public function getInvoiceStatusMultiplePlan($seller_id){
			$query=$this->db->query("SELECT pvpi.status_id AS invoice_status FROM " . DB_PREFIX . "purpletree_vendor_plan_invoice pvpi LEFT JOIN " . DB_PREFIX . "purpletree_vendor_seller_plan pvsp ON (pvpi.invoice_id = pvsp.invoice_id) WHERE pvsp.seller_id='".(int) $seller_id."' AND pvsp.new_status=1");
			if($query->num_rows){	
				return $query->rows;
				} else {
				return NULL;	
		...
		public function getSsellerplanStatusMultiplePlan($seller_id) {
			//$query=$this->db->query("SELECT status_id FROM ". DB_PREFIX ."purpletree_vendor_plan_subscription WHERE seller_id='".(int)$seller_id."'");
		...
		public function updateSeenContact($seller_id,$customerid){
			$this->db->query("UPDATE `" . DB_PREFIX . "purpletree_vendor_contact` SET seen = '0' WHERE seller_id = '" . (int)$seller_id . "' AND customer_id='".(int)$customerid."'");
		}
		public function checkSellerVacations($seller_id){		
