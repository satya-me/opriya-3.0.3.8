...
class ModelExtensionPurpletreeMultivendorGeozone extends Model {
		...
		public function getGeoZone($geo_zone_id) {
			$query = $this->db->query("SELECT CONCAT(c.firstname, ' ', c.lastname) AS seller_name, gz.*,pvg.* FROM " . DB_PREFIX . "geo_zone gz LEFT JOIN " . DB_PREFIX . "purpletree_vendor_geozone pvg ON (gz.geo_zone_id = pvg.geo_zone_id) LEFT JOIN " . DB_PREFIX . "customer c ON (c.customer_id = pvg.seller_id) WHERE gz.geo_zone_id = '" . (int)$geo_zone_id . "'");
		...
		public function getGeoZones($data = array()) {
			if ($data) {
				$sql = "SELECT CONCAT(c.firstname, ' ', c.lastname) AS seller_name, pvg.*, gz.* FROM " . DB_PREFIX . "geo_zone gz INNER JOIN " . DB_PREFIX . "purpletree_vendor_geozone pvg ON (gz.geo_zone_id = pvg.geo_zone_id) INNER JOIN " . DB_PREFIX . "customer c ON (c.customer_id = pvg.seller_id) AND pvg.seller_id !=0 AND pvg.seller_id = '" . (int)$this->customer->getId() . "'";
		...
		public function getTotalGeoZones($seller_id) {
			$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "purpletree_vendor_geozone WHERE seller_id = '" . (int)$seller_id . "'");
