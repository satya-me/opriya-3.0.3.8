...
class ModelExtensionCatalogOption extends Model {
		...
		public function getOptions($data = array(),$stringgattrsets ='') {
			$attribbb ='';
			if($stringgattrsets != '') {
				$attribbb = " AND o.option_id NOT IN (".$stringgattrsets.") ";
			}
			$sql = "SELECT * FROM `" . DB_PREFIX . "option` o LEFT JOIN " . DB_PREFIX . "option_description od ON (o.option_id = od.option_id) WHERE od.language_id = '" . (int)$this->config->get('config_language_id') . "'". $attribbb  ."";
