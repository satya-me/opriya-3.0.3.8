<?php
class ControllerWebExtensionAccountPurpletreeMultivendorBulkproductupload extends Controller{
		private $error = array();
		public function index(){
			...
			if (!$this->customer->isLogged()) {
				$this->session->data['redirect'] = $this->url->link('web/extension/account/purpletree_multivendor/bulkproductupload', '', true);
		...
		public function export(){
			...
			if(!$this->customer->validateSeller()) {
				...
				$this->response->redirect($this->url->link('web/extension/account/purpletree_multivendor/bulkproductupload', '', true));
			}
										...
										if($key!=1) {	
											...
											try {
												if(is_numeric($data['product_id']) && $data['product_id']!='' && !is_string($data['product_id']))
												{
													...
													$imagesourc = '';
													if($sourcecode !== FALSE) {							
														$imagesourc = 'catalog/Seller_'.$sseller_id.'/'.basename(trim($data['image']));
													}
				...
				$exp= explode(',',preg_replace('/^\p{Z}+|\p{Z}+$/u', '', $value['category']));
					...
					foreach($exp as $vvv){
						...
						if(!empty($impData)){
							...
							$lastids		= $this->model_extension_purpletree_multivendor_bulkproductupload->getCategoryIds(preg_replace('/^\p{Z}+|\p{Z}+$/u', '', $lastname),$this->request->post['language']);
							$dddd = array();
							if(!empty($lastids)) {
						...
						if(!empty($dddd)) {
							//
							$secarray = array();
							$selass		= $this->model_extension_purpletree_multivendor_bulkproductupload->getCategoryIds(preg_replace('/^\p{Z}+|\p{Z}+$/u', '', $secondLast),$this->request->post['language']);
							if(!empty($selass)) {
							...
							//When 1,2nd child category found in sheet
								$category_id = $secarray;
							} else {
							//When 3rd child category found in sheet
							$firstids = $this->model_extension_purpletree_multivendor_bulkproductupload->getCategoryIds(preg_replace('/^\p{Z}+|\p{Z}+$/u', '', $thirdast),$this->request->post['language']);
							if(!empty($firstids)) {
...
/* -------------------------------------/CATEGORY----------------------------------------------------- */
											...
											if(!in_array($valueImage['product_id'],$failed_array)) {
												...
												if(in_array($product_id,$passed_array)) {	
													...
													if($sourcecode !== FALSE) {
														...
														'product_image_id'			=>		$valueImage['product_image_id'],
														'product_id'				=>		$product_id,
														'image'						=>		'catalog/Seller_'.$sseller_id.'/'.basename(trim($valueImage['image'])),
						...
						$url='';
						$this->response->redirect($this->url->link('web/extension/account/purpletree_multivendor/bulkproductupload', '' . $url, true));
						} else {
		...
		protected function getData() {
			...
			'href' => $this->url->link('web/common/dashboard', '', true)
			);
			$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('web/extension/account/purpletree_multivendor/bulkproductupload', '' . $url, true)
			);	
			$this->load->model('extension/purpletree_multivendor/dashboard');
			$this->model_extension_purpletree_multivendor_dashboard->checkSellerApproval();
			...
			$data['action'] = $this->url->link('web/extension/account/purpletree_multivendor/bulkproductupload/upload', '', true);	
			$data['export'] = $this->url->link('web/extension/account/purpletree_multivendor/bulkproductupload/export', '', true);
			...
			$this->response->setOutput($this->load->view('web/account/purpletree_multivendor/bulkproduct_upload', $data));		
		}
	}
