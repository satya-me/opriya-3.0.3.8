<?php
class ControllerWebExtensionAccountPurpletreeMultivendorApiSellercontact extends Controller {
		private $error = array();
		...
		protected function validate($requestjson1) {
			...
			if (!isset($requestjson1['customer_name']) || (utf8_strlen($requestjson1['customer_name']) < 3) || (utf8_strlen($requestjson1['customer_name']) > 32)) {
				$this->error['customer_name'] = $this->language->get('error_name');
			}
			$EMAIL_REGEX='/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$/';
			if (!isset($requestjson1['customer_email']) || preg_match($EMAIL_REGEX, $requestjson1['customer_email'])==false) {
				$this->error['customer_email'] = $this->language->get('error_email');
			}
			
			if (!isset($requestjson1['customer_message']) || (utf8_strlen($requestjson1['customer_message']) < 10) || (utf8_strlen($requestjson1['customer_message']) > 3000)) {
				$this->error['customer_message'] = $this->language->get('error_enquiry');
			}
