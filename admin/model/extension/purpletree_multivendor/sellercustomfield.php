<?php
class ModelExtensionPurpletreeMultivendorSellercustomfield extends Model {
	public function addCustomField($data) {
		...
		$this->db->query("INSERT INTO `" . DB_PREFIX . "purpletree_vendor_customfield` SET custom_field_id = '" . (int)$custom_field_id . "', step_location = '" . $this->db->escape($data['step_location']) . "'");
	...
	public function editCustomField($custom_field_id, $data) {
		...
		$this->db->query("UPDATE `" . DB_PREFIX . "purpletree_vendor_customfield` SET step_location = '" . $this->db->escape($data['step_location']) . "' WHERE custom_field_id = '" . (int)$custom_field_id . "'");
	...
	public function deleteCustomField($custom_field_id) {
		...
		$this->db->query("DELETE FROM `" . DB_PREFIX . "custom_field_value` WHERE custom_field_id = '" . (int)$custom_field_id . "'");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "custom_field_value_description` WHERE custom_field_id = '" . (int)$custom_field_id . "'");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "purpletree_vendor_customfield` WHERE custom_field_id = '" . (int)$custom_field_id . "'");
	}

	public function getCustomField($custom_field_id) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "custom_field` cf LEFT JOIN " . DB_PREFIX . "custom_field_description cfd ON (cf.custom_field_id = cfd.custom_field_id) INNER JOIN " . DB_PREFIX . "purpletree_vendor_customfield pvc ON (pvc.custom_field_id = cfd.custom_field_id) WHERE cf.custom_field_id = '" . (int)$custom_field_id . "' AND cfd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
	...
	public function getCustomFields($data = array()) {
		if (empty($data['filter_customer_group_id'])) {
			$sql = "SELECT * FROM `" . DB_PREFIX . "custom_field` cf LEFT JOIN " . DB_PREFIX . "custom_field_description cfd ON (cf.custom_field_id = cfd.custom_field_id) INNER JOIN " . DB_PREFIX . "purpletree_vendor_customfield pvc ON (pvc.custom_field_id = cf.custom_field_id) WHERE cfd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		} else {
			$sql = "SELECT * FROM " . DB_PREFIX . "custom_field_customer_group cfcg LEFT JOIN `" . DB_PREFIX . "custom_field` cf ON (cfcg.custom_field_id = cf.custom_field_id) LEFT JOIN " . DB_PREFIX . "custom_field_description cfd ON (cf.custom_field_id = cfd.custom_field_id) INNER JOIN " . DB_PREFIX . "purpletree_vendor_customfield pvc ON (pvc.custom_field_id = cf.custom_field_id) WHERE cfd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		}
