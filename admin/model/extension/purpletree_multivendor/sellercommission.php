...
class ModelExtensionPurpletreeMultivendorSellercommission extends Model{
		...
		public function getCommissions($data=array()){
			...
			$sql = "SELECT pvc.*,pd.name,op.name AS order_product_name,pvo.total_price,o.currency_code, o.currency_value,pvs.id AS store_id,pvs.store_name FROM " . DB_PREFIX . "purpletree_vendor_commissions pvc JOIN " .DB_PREFIX. "product_description pd ON(pd.product_id=pvc.product_id) JOIN " .DB_PREFIX. "purpletree_vendor_orders pvo ON(pvo.product_id=pvc.product_id AND pvo.order_id=pvc.order_id AND pvo.id=pvc.vendor_order_table_id) JOIN `" .DB_PREFIX. "order` o ON(o.order_id=pvo.order_id) JOIN `" .DB_PREFIX. "purpletree_vendor_stores` pvs ON(pvs.seller_id=pvc.seller_id) JOIN " .DB_PREFIX. "order_product op ON(op.product_id=pvc.product_id)";
