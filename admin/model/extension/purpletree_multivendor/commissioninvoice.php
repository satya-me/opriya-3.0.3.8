...
class ModelExtensionPurpletreeMultivendorCommissioninvoice extends Model{
		...
		public function getOrderProducts($commisionid){		
			...
			$sql = "SELECT pvo.*,pd.name,op.order_product_id FROM " . DB_PREFIX . "purpletree_vendor_orders pvo JOIN " .DB_PREFIX. "product_description pd ON(pd.product_id=pvo.product_id) JOIN " .DB_PREFIX. "order_product op ON(op.order_id=pvo.order_id AND op.price=pvo.unit_price) ";
